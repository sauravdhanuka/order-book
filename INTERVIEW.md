...
**A:** TCP is a stream protocol — a `recv()` call might return fewer bytes than requested. If I send 32 bytes, the server might receive it as two reads of 16 bytes each. This is especially common under high load.
...
### Q: Why `select()` for the event loop? Isn't that slow?

**A:** I chose `select()` because it's the only I/O multiplexing API that works on all three platforms — Windows, macOS, and Linux — without any `#ifdef` in the event loop code. The alternatives are all platform-specific:

| Approach | Portability | Scalability |
|----------|------------|-------------|
| `select()` | Windows + macOS + Linux | O(n) scan, 1024 FD limit on some systems |
| `kqueue` | macOS/BSD only | O(1) per event |
| `epoll` | Linux only | O(1) per event |
| `IOCP` | Windows only | O(1) per event |

For this project — a demo with a handful of clients — `select()` adds zero measurable overhead. The matching engine processes orders in ~100ns; the `select()` overhead for 5-10 file descriptors is negligible.

In a production system handling thousands of connections, I'd use the platform-native API (`epoll` on our Linux trading servers) or a cross-platform library like `libuv` or `io_uring`. But for demonstrating the matching engine, portability matters more than scaling to 10,000 connections.

### Q: How did you handle the cross-platform differences in socket APIs?

**A:** I created a thin abstraction layer in `platform.h` that wraps the differences:

| Operation | Windows (Winsock2) | POSIX (macOS/Linux) |
|-----------|-------------------|---------------------|
| Socket type | `SOCKET` (unsigned 64-bit) | `int` |
| Close socket | `closesocket(fd)` | `close(fd)` |
| Read | `recv(fd, buf, len, 0)` | `read(fd, buf, len)` |
| Write | `send(fd, buf, len, 0)` | `write(fd, buf, len)` |
| Non-blocking | `ioctlsocket(fd, FIONBIO, &mode)` | `fcntl(fd, F_SETFL, O_NONBLOCK)` |
| Error check | `WSAGetLastError() == WSAEWOULDBLOCK` | `errno == EAGAIN` |
| Startup | `WSAStartup()` required | Nothing needed |
| Shutdown signal | `SetConsoleCtrlHandler()` | `signal(SIGINT, handler)` |

Each of these is a one-line inline function in `platform.h`. The rest of the codebase uses `socket_t`, `platform_recv()`, `platform_send()`, etc. — no `#ifdef` leaks into the TCP server or client logic.

Key design choice: I use a `socket_t` typedef (`SOCKET` on Windows, `int` on POSIX) and `INVALID_SOCK` constant (`INVALID_SOCKET` on Windows, `-1` on POSIX) so the server code doesn't need to know which platform it's on.
...
**A:** This is a subtle correctness issue. The event loop uses `select()` to identify ready fds, then iterates over clients:

```cpp
// Copy client list since handle_client_data may modify clients_
std::vector<socket_t> client_fds;
for (const auto& c : clients_) client_fds.push_back(c.fd);
...
The key safety measure: we snapshot the fd list *before* iterating. `handle_client_data` may call `remove_client`, which erases from the `clients_` vector. But we're iterating over `client_fds` (the snapshot), not `clients_` directly. So the vector mutation doesn't invalidate our iterator.

The potential issue: if `remove_client(fd)` closes the fd, and then `accept_client()` (from the listener check) reuses that same fd number for a new connection, a subsequent `FD_ISSET` for the old fd might match the new fd. This is handled because `handle_client_data` uses `std::find_if` to locate the client by fd — if the old client was removed, the new client's `bytes_read` is 0, so it's treated as a fresh connection (correct behavior).
