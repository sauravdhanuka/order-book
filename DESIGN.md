...
- C++20 compiler (Clang 14+, GCC 12+, or MSVC 2022 17.0+)
- CMake 3.20+
...
Binaries will be in `build\Release\` on Windows instead of `build/` on Unix.

> **Note:** The core matching engine, order book, CSV parser, benchmarks, and tests are 100% portable C++20 with no platform-specific code. Only the TCP server/client use platform abstractions (Winsock on Windows, POSIX sockets on macOS/Linux), handled via `include/platform.h`.
...
│   ├── platform.h                # Cross-platform socket & signal abstractions
...
**Cross-platform I/O multiplexing using `select()`:**

The server uses `select()` for I/O multiplexing — it works on macOS, Linux, and Windows without any platform-specific code. All socket operations (connect, recv, send, close, non-blocking mode) go through a thin abstraction layer in `platform.h` that maps to Winsock2 on Windows and POSIX sockets on Unix.
...
`select()` monitors a set of file descriptors and returns when any are ready for reading. Compared to platform-specific alternatives (`kqueue` on macOS, `epoll` on Linux, `IOCP` on Windows), `select()` is less scalable for thousands of connections but perfectly sufficient for this project and works everywhere.
...
For a production system handling thousands of connections, you'd use the platform-native API or a library like `libuv`. For this demo with a handful of clients, `select()` adds zero measurable overhead.
...
TCP is a stream protocol — a `recv()` call might return 10 bytes of a 32-byte message. The `bytes_read` counter tracks how much of the current message has been received. Once 32 bytes are accumulated, the message is deserialized and processed. This handles TCP fragmentation correctly.

**Graceful shutdown (cross-platform):**

```cpp
// platform.h handles this portably:
// - POSIX: signal(SIGINT, handler) + sig_atomic_t flag
// - Windows: SetConsoleCtrlHandler() + InterlockedExchange() flag
...
7. **`select()` scalability.** The TCP server uses `select()` for portability, which has an O(n) scan and a 1024-FD limit on some systems. For high connection counts, a production system would use `epoll` (Linux), `kqueue` (macOS), or `IOCP` (Windows).
